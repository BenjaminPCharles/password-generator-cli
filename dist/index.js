...
const validTypes = ['alpha', 'hex', 'strong'];
program
    .name('password')
    .description('Generate random password')
    .version('v1.0.0')
    .option('-l, --length <length>', 'Password length', '16')
    .option('-t, --type <type>', `Password type: (${validTypes.join(' | ')})`, 'alpha')
    ...
    if (!validTypes.includes(type)) {
        console.error(chalk.red(`❌ Invalid type. Use one of: ${validTypes.join(', ')}`));
        process.exit(1);
    }
    /**
     * Generate password
    ...
    console.log(`${chalk.gray('➡️ ')} ${chalk.bold.cyan(password)} ` +
